...
2. **GitHub 클론만** — AGENTS의 **제품 제약**만 지키면 된다. 인증 끄기, 원문 서버 금지, Remmikim-bit 커밋. 미리보기 프록시/Grok 필은 그 환경에 해당할 때만.
...
- ChatGPT / Claude / Grok / Gemini **대화 내보내기 JSON**을 브라우저에서 파싱한다.
...
- 소셜 비교 / 리더보드 / 계정 / 클라우드 을 제공할 예정이다.
- n8n 백엔드 없음. 초기에 사용자가 물어봤고, “제로 비용 + 프라이버시”로 **거절된** 방향이다. 다시 제안하지 마라.
- 키도 없다. 문장 다듬기는 **grok-4.5 + `XAI_API_KEY`**, Gemini Flash 만.
...
미리보기에서 말해야 할 것: “AI 대화내용을 분석하고 [여러가지 기능들의 타이포 그래피가 바뀌는 모습]를 확인해 보세요!” 
폰트를 다르게 해서 여러가지 기능들을 감각적으로 소개해야 한다.
