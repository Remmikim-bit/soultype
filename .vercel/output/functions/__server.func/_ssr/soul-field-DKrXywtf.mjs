...
//#region node_modules/.nitro/vite/services/ssr/assets/soul-field-DKrXywtf.js
...
function idleParams(t) {
	...
	return {
		verts: .15 + 3.8 * s(.19, .15) + 2.1 * s(.11, 1.3),
		sharp: .06 + .82 * s(.15, 1.6),
		hull: .04 + .84 * s(.13, .5),
		size: .62 + .08 * Math.sin(t * .21),
		warp: .16 + .38 * s(.17, 2),
		stretch: .12 + .78 * s(.2, .8)
	};
}
function paramsFromAxes(axes) {
	...
	return {
		...
		size: .58 + (axes.ie + 1) / 2 * .18,
		warp: .06 + dream * .5 + feel * .12,
...
function SoulField({ stage, axes, locked, quadrant, caption }) {
	...
	(0, import_react.useEffect)(() => {
		...
		const tick = (now) => {
			...
			const want = p.stage === "gate" ? .94 : .84;
			presence = damp(presence, want, dt, .7);
