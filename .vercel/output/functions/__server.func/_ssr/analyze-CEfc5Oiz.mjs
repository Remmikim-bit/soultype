...
import { c as classifyLocal, o as characterOf, r as QUADRANTS } from "./mbti-local-Di_mXfG-.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/analyze-CEfc5Oiz.js
...
var SYSTEM = `너는 사용자의 대화 습관이 쌓여 AI가 학습·변모한 결과를 읽는 카피라이터다.
...
function coerce(raw, fallback) {
	...
	const o = raw;
	const mbti = String(o.mbti ?? fallback.mbti).toUpperCase().replace(/[^EISNTFJP]/g, "").slice(0, 4);
...
var analyzeUsage_createServerFn_handler = createServerRpc({
	id: "74162b8540f4d23bb11989c8205a4ace5e185bea902ddd393832c55bd8596975",
	name: "analyzeUsage",
	filename: "src/lib/analyze.ts"
}, (opts) => analyzeUsage.__executeServer(opts));
var analyzeUsage = createServerFn({ method: "POST" }).validator((input) => capDigest(input)).handler(analyzeUsage_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		const parsed = coerce(extractJson((await res.json()).choices?.[0]?.message?.content ?? ""), fallback);
...
//#endregion
export { analyzeUsage_createServerFn_handler };
